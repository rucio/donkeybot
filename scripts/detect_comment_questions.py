...
def main():
    # Parse cli arguments
    parser = argparse.ArgumentParser(description='''Run this script to detect and save questions originating from GitHub issues''')
    ...
    for i in tqdm(range(len(comments_df.index))):
        ...
        if not questions_detected: 
            ...
            for question in questions_detected:
                ...
                question.find_context_from_table(data_storage, table_name=comments_table)
                if question.context == '':
                    continue
                else:
