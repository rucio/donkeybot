...
def main():
    # Parse cli arguments
    parser = argparse.ArgumentParser(description='''Run this script to detect and save questions originating from emails''')
    ...
    conv_df = emails_df[emails_df['conversation_id'].notnull()].sort_values(by=['conversation_id', 'email_date']).reset_index(drop=True)
