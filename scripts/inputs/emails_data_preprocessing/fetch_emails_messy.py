...
def _get_bodies(emlx, decode=False, decode_format="ISO-8859-1"):
    ...
    for part in emlx.walk():
        ...
        b = _decode(b, decode_format=decode_format)  # "unicode_escape", "ISO-8859-1", "UTF-8"
        bdies.append(b)
    return bdies
...
def process_email(item, mail=None):
    ...
    def get_hashes_from_mails(mails):
        ...
        for m in mail_list:
            ...
            if 'support' in nickname: continue
            if 'rucio' in nickname: continue
            if 'atlas' in nickname: continue
    ...
    unwrapped_bodies = _get_bodies(item, decode=True, decode_format='ISO-8859-1')  # "unicode_escape", "ISO-8859-1", "utf-8"
    ...
    for body in unwrapped_bodies:
        ...
        _names_f, sender = _remove_names('from ' + email_message['From'], dic=sender_dict)
        _names_t, receiver = _remove_names('to ' + email_message['To'], dic=receivers_dict)
    ...
    for filepath in glob.iglob(inbox + '/**', recursive=True):
        ...
        if filepath.endswith('.emlx'):
            try:
                ...
                if body_count == 0:
                    logger.debug('This email seems to have faulty body: {}'.format(type(body)))
                    counter_no_body +=1
...
while counter<lmt:
     ...
     try:
         ...
         search_fetch_local('/Users/javor/Desktop/DDM/RUCIODEV/NLTK/MAILanal/IMAP_client/emails_data_preprocessing/DDM.mbox')
