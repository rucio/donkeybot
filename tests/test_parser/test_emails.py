...
def test_email_data_saved_on_db(parsed_email, test_email_in_db):
    ...
    assert test_email_in_db[5] == parsed_email.date.strftime("%Y-%m-%d %H:%M:%S+00:00")  # email_date
...
def test_parsed_email_id(parsed_email):
    assert parsed_email.id == 1 # only one email exists in the test_db (the test_email from the fixture)
...
def test_multiple_senders_error(test_email, email_parser):
    ...
    with pytest.raises(SystemExit): # raises SystemExit because of sys.exit(MultipleSendersError)
        parsed_other_test_email = email_parser.parse(**other_test_email)
...
def test_find_conversation_hashing(test_email, email_parser):
    ...
    update_to_art_contained_cid = "cid_"+hashlib.md5(clean_subject.encode("utf-8")).hexdigest()[:6]
...
def test_cid_not_created_for_first_email(test_email, email_parser):
    other_test_email = {k:v for k,v in test_email.items()} 
    other_test_email["subject"] = "first email subject where a conv id should not be created or exist"
    find_conversation_hash = email_parser.find_conversation(other_test_email["subject"], dict_name="test_dict")
...
def test_cid_not_created_for_fwd_email(test_email, email_parser):
    other_test_email = {k:v for k,v in test_email.items()}
    other_test_email["subject"] = "FWD: fwd email subject where a conv id should not be created or exist"
    find_conversation_hash = email_parser.find_conversation(other_test_email["subject"], dict_name="test_dict")
    # make sure hash not found
    assert find_conversation_hash == None 
...
def test_parsed_email_date(test_email, parsed_email):
    ...
    assert parsed_email.date == utils.convert_to_utc(test_email["date"], "%a, %d %b %Y %H:%M:%S %z")
...
def test_clean_body_stops_at_first_regex(email_parser):
    body = "correct text >>>>  you should not see this -----Original Message----- or this"
    cleaned_body = email_parser.clean_body(body)
    assert cleaned_body == "correct text "
